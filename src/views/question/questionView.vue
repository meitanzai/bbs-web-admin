    ...
    <div class="main blankBackground" >
        ...
        <div class="questionViewModule" v-if="state.question != undefined && state.question != null && Object.keys(state.question).length>0">
            <div class="question-wrap">
                ...
                <div class="operat">
                    <el-link class="item" href="javascript:void(0);" v-if="state.question.status == 10" @click="auditQuestion(state.question.id);">审核</el-link>
                    <el-link class="item" href="javascript:void(0);" @click="$router.push({path: '/admin/control/questionFavorite/list', query:{ visible:($route.query.visible != undefined ? $route.query.visible:''),questionView_beforeUrl:($route.query.questionView_beforeUrl != undefined ? $route.query.questionView_beforeUrl:''),questionId :state.question.id, answerId:($route.query.answerId != undefined ? $route.query.answerId:''),page:($route.query.page != undefined ? $route.query.page:''), questionPage:($route.query.page != undefined ? $route.query.page:'')}})">收藏用户</el-link>
                    ...
                    <el-link class="item" href="javascript:void(0);" @click="deleteQuestion(state.question.id)">删除</el-link>
                </div>
                <!-- 追加提问 -->
                    ...
                    <div class="editQuestion data-form">
                        <el-form label-width="120"  @submit.native.prevent>
                            <el-form-item label="标题" :required="true" :error="error.title">
                                <el-input v-model.trim="state.title" maxlength="100" :clearable="true" show-word-limit></el-input>
                            </el-form-item>
                    ...
                    <div class="questionInfo clearfix" >
                        <div class="avatar">
                            <el-popover effect="light" trigger="hover" placement="top">
                                <template #default>
                                    <p v-if="state.question.isStaff == false">呢称: {{state.question.nickname}}</p>
                    ...
                    <div class="editAppendQuestion-post" v-show="state.editAppendQuestionFormView.get(appendQuestionItem.id)">
                        <div class="editAppendQuestion-formModule">
                            <div class="editAppendQuestion-wrap">
                                ...
                                <textarea :ref="el => state.editAppendQuestionElementNodes[index]=el" :appendQuestionItemId="appendQuestionItem.id" class="editAppendQuestion-textarea" ></textarea>
        ...
        <div class="answerModule">
            <div class="answerList">
                <div :class="answer.adoption ? 'item activeItem' : 'item'" v-for="(answer,index) in state.answerList" :key="answer.id" :answerId="answer.id"  :ref="'answer_'+answer.id">
                    <div class="head">
                        <div class="avatarBox">
                            <el-popover effect="light" trigger="hover" placement="top">
                                <template #default>
                                    ...
                                    <div class="editAnswerReply-formModule" v-show="state.editReplyFormView.get(reply.id)">
                                        <div class="editReply-wrap">
                                            <el-form label-width="120"  @submit.native.prevent>
                                                ...
                                                <el-form-item label=" ">
                                                    <el-button class="submitButton" type="primary" size="large" @mousedown.native="editReply(reply.id)" :disabled="state.editReply_disabled.get(reply.id)">提交</el-button>
                                                    <el-button class="submitButton" type="primary" size="large" plain @mousedown.native="cancelEditReply(reply.id);">取消</el-button>
                                                </el-form-item>
                                            </el-form>
                                        </div>
                                    </div>
                    ...
                    <div class="addReply-post" v-show="state.addReplyFormView.get(answer.id)">
                        <el-form @submit.native.prevent>
                            <el-form-item >
                                <el-input type="textarea" :autosize="{minRows: 5}" placeholder="请输入内容" v-model="state.addReplyContentField[index]"></el-input>
                            </el-form-item>
                            <el-form-item>
                                <el-button class="submitButton" type="primary" size="large" @mousedown.native="addReply(answer.id)" :disabled="state.addReply_disabled.get(answer.id)">提交</el-button>
                                <el-button class="submitButton" type="primary" size="large" plain @mousedown.native="cancelAddReply(answer.id);">取消</el-button>
                            </el-form-item>
                        </el-form>
                    </div>
                ...
                <div class="addAnswer" >
                    <el-form @submit.native.prevent>
                        ...
                        <el-form-item>
                            <el-button class="submitButton" type="primary" size="large" @mousedown.native="addAnswerForm" :disabled="state.addAnswerForm_disabled">提交</el-button>
...
<script lang="ts" setup>
    ...
    import { processErrorInfo,getLanguageClassName, updateURLParameter, getElementTop } from '@/utils/tool';
                    ...
                    if(returnValue.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			if(key == "question"){//问题
			    				...
			    				state.editReplyStatusField.length = 0;
			    				...
			    				if(answerList != null && answerList.length > 0){
				    				for (let i = 0; i <answerList.length; i++) {
										...
										//alert(b[0][1]);//2
										state.editReplyStatusField[i] = editReplyStatusField_reply_array;//把数组editReplyStatusField_reply_array作为editReplyStatusField数组的元素传入editReplyStatusField数组中
										state.editReplyContentField[i] = editReplyContentField_reply_array;
    ...
    const addReply = (answerId:string) => {
        ...
        if(state.answerList != null && state.answerList.length > 0){
            for (let i = 0; i <state.answerList.length; i++) {
                let answer = state.answerList[i];
                if(answer.id == answerId){
                    if(state.addReplyContentField[i] != undefined){
            ...
            for (let i = 0; i <state.answerList.length; i++) {
                ...
                if(answer.answerReplyList != null && answer.answerReplyList.length >0){
                    for (let j = 0; j <answer.answerReplyList.length; j++) {
                        let reply = answer.answerReplyList[j];
                        if(reply.id == replyId){
                            formData.append('status', state.editReplyStatusField[i][j]); 
                            if(state.editReplyContentField [i][j] != undefined){
