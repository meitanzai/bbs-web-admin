...
<template>
    <div class="main">
        ...
        <div class="navbar">
            <el-button type="primary" plain @click="$router.push({path: '/admin/control/layout/manage/add',query:{ dirName : $route.query.dirName}});">添加布局</el-button>
        </div>
        <div class="data-table" >
            <el-table ref="multipleTable" :data="state.tableData" tooltip-effect="dark" style="width: 100%" stripe empty-text="没有内容">
                ...
                <el-table-column label="操作" align="center" width="330">
                    <template #default="scope">
                        <el-button-group>
                            <el-button type="primary" @click="$router.push({path: '/admin/control/forum/list', query:{ layoutId : scope.row.id,dirName : $route.query.dirName,sourcePage:($route.query.page != undefined ? $route.query.page:'')}})">版块</el-button>
                            <el-button type="primary" v-if="scope.row.type !=6 && scope.row.returnData != 1" @click="$router.push({path: '/admin/control/layout/manage/editLayoutCode', query:{ layoutId : scope.row.id,dirName : $route.query.dirName,page : ($route.query.page != undefined ? $route.query.page:'')}})">布局代码编辑</el-button>
                            <el-button type="primary" @click="$router.push({path: '/admin/control/layout/manage/editLayout', query:{ layoutId : scope.row.id,dirName : $route.query.dirName,page : ($route.query.page != undefined ? $route.query.page:'')}})">修改</el-button>
                            <el-button type="primary" @click="deleteLayout($event,scope.row)">删除</el-button>
                        </el-button-group>
